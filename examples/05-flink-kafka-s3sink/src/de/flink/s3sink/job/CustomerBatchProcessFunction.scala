 ...
 *   2. The raw order is appended to per-customer Flink state, tagged with that window.
 *   3. An *event-time* timer is registered for the last millisecond of the window. Flink fires it when the watermark
 *      passes that timestamp, not when the wall clock does. Replaying a week of history therefore produces exactly the
 *      same windows as reading the topic live.
...
final class CustomerBatchProcessFunction(windowSizeMillis: Long)
  ...
  override def processElement(
    ...
    ctx.timerService().registerEventTimeTimer(windowStart + windowSizeMillis - 1L)
  }
