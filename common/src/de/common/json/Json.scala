 ...
 *
 * JSON (JavaScript Object Notation) is the wire format used by most examples. A full JSON library (circe, jsoniter,
 * upickle, ...) cannot be used here because this module is compiled for three different Scala versions and no single
 * library version covers all of them. Encoding is all this module needs; every example that also has to *parse* JSON
 * pulls in a real library.
 */
object Json {
